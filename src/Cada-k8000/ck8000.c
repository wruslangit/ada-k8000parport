...
*  Delay functions.                                            *
\**************************************************************/
static inline void I2cDelay(void)
{
	int i,j;
	for (i=0;i<I2CbusDelay;i++) {j++;};
}

static inline void I2cPreDelay(void)
{
	int i,j;
	for (i=0;i<pre_I2CbusDelay;i++) {j++;};
}

static inline void I2cPostDelay(void)
{
	int i,j;
...
\**************************************************************/
#ifdef __MPC860__
static inline byte GetScl(void)
{
	byte tmp;
...
static inline void SetScl(short state)
{
	volatile cpm8xx_t *cp = cpmp;
	int i=0;
...
static inline void SetSda(short state)
{
	volatile cpm8xx_t *cp = cpmp;
...
static inline byte GetSda(void)
{
	byte tmp;
	volatile cpm8xx_t *cp = cpmp;
...
static inline void SdaOut(void)
{
	volatile cpm8xx_t *cp = cpmp;
...
\**************************************************************/
static inline void StartCond(void)
{
...
static inline void StopCond(void)
{
	SetSda(LOW);
	SetScl(HIGH);
	SetSda(HIGH);
...
static inline byte GetAck(void)
{
	int i;
	byte val;
...
static inline void MasterAck(void)
{
	SetSda(LOW);
	SetScl(HIGH);
	SetScl(LOW);
...
static inline void MasterNack(void)
{
	SetSda(HIGH);
	SetScl(HIGH);
	SetScl(LOW);
